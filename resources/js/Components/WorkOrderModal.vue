<template>
  <div class="modal fade show d-block" tabindex="-1">
    <div class="modal-dialog modal-lg">
      <div class="modal-content">
        <!-- Modal Header -->
        <div class="modal-header bg-primary text-white">
          <h5 class="modal-title">Work Order Details</h5>
          <button type="button" class="close" @click="$emit('close')">&times;</button>
        </div>
        <!-- Modal Body -->
        <div class="modal-body" v-if="order">
          <div class="row">
            <div class="col-md-6">
              <div class="form-group">
                <label>Ticket Number:</label>
                <p>{{ order.ticket_number }}</p>
              </div>
            </div>
            <div class="col-md-6">
              <div class="form-group">
                <label>Requested By:</label>
                <p>{{ order.requested_by }}</p>
              </div>
            </div>
          </div>
          <div class="row">
            <div class="col-md-6">
              <div class="form-group">
                <label>Requisitioner Type:</label>
                <p>{{ order.requisitioner_type }}</p>
              </div>
            </div>
            <div class="col-md-6">
              <div class="form-group">
                <label>College/Unit:</label>
                <p>{{ order.college }}</p>
              </div>
            </div>
          </div>
          <div class="row">
            <div class="col-md-6">
              <div class="form-group">
                <label>Department:</label>
                <p>{{ order.department }}</p>
              </div>
            </div>
            <div class="col-md-6">
              <div class="form-group">
                <label>Concern:</label>
                <p>{{ order.concern }}</p>
              </div>
            </div>
          </div>
          <div class="row">
            <div class="col-md-6">
              <div class="form-group">
                <label>Description:</label>
                <p>{{ order.description }}</p>
              </div>
            </div>
            <div class="col-md-6">
              <div class="form-group">
                <label>Date Requested:</label>
                <p>{{ order.date_requested }}</p>
              </div>
            </div>
          </div>
          <div class="row">
            <div class="col-md-6">
              <div class="form-group">
                <label>Status:</label>
                <p>{{ order.status }}</p>
              </div>
            </div>
            <!-- If order is Completed, show Category -->
            <div class="col-md-6" v-if="order.status === 'Completed'">
              <div class="form-group">
                <label>Category:</label>
                <p>{{ order.category }}</p>
              </div>
            </div>
          </div>
          <!-- If order is Received or Completed, allow entering Completed Description -->
          <div v-if="order.status === 'Received' || order.status === 'Completed'" class="row">
            <div class="col-md-12">
              <div class="form-group">
                <label>Completed Description:</label>
                <textarea
                  v-model="order.completed_description"
                  class="form-control"
                  placeholder="Enter description of completed work"
                  required
                ></textarea>
              </div>
            </div>
          </div>
        </div>
        <!-- Modal Footer: Action Buttons -->
        <div class="modal-footer">
          <button
            v-if="order.status === 'Submitted'"
            class="btn btn-success"
            @click="$emit('accept', order)"
          >
            Accept
          </button>
          <button
            v-if="order.status === 'Submitted'"
            class="btn btn-danger"
            @click="$emit('decline', order)"
          >
            Decline
          </button>
          <button
            v-if="order.status === 'Received'"
            class="btn btn-success"
            @click="$emit('complete', order)"
          >
            Complete
          </button>
          <button class="btn btn-secondary" @click="$emit('print')">Print</button>
          <button class="btn btn-secondary" @click="$emit('close')">Close</button>
        </div>
      </div>
    </div>
  </div>
</template>

<script setup>
const props = defineProps({
  order: {
    type: Object,
    default: null
  }
});
</script>

<style scoped>
.modal-header {
  background-color: #007bff; /* Primary color */
}
.modal-title {
  font-weight: bold;
}
.form-group {
  margin-bottom: 1rem;
}
.badge {
  font-size: 14px;
  padding: 5px 10px;
}
button {
  cursor: pointer;
}
.btn-outline-info {
  color: #17a2b8;
  border-color: #17a2b8;
}
.btn-outline-info:hover {
  background-color: #17a2b8;
  color: #fff;
}
</style>
